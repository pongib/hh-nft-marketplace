...
!developmentChainId.includes(network.config.chainId!)
  ...
  : describe("Nft Marketplace Unit Tests", function () {
      ...
      describe("listItem", function () {
        ...
        it("exclusively items that haven't been listed", async function () {
          ...
          await expect(
            ...
            .to.be.revertedWithCustomError(
              ...
              "NftMarketplace__NftAlreadyList"
            )
        ...
        it("needs approvals to list item", async function () {
          ...
          await expect(
            ...
            nftMarketplaceContract,
            "NftMarketplace__NotApprovForMarketplace"
          )
        })
      ...
      describe("buyItem", function () {
        ...
        it("reverts if the price isnt met", async function () {
          ...
          await expect(
            ...
            nftMarketplaceContract,
            "NftMarketplace__NotEnoughEthToBuy"
          )
        })
        it("transfers the nft to the buyer and updates withdraw fund record", async function () {
          await nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
          nftMarketplace = nftMarketplaceContract.connect(user)
          expect(
          ...
          const deployerProceeds = await nftMarketplace.getSellerFund(
      ...
      describe("withdrawSellerFund", function () {
        it("doesn't allow 0 proceed withdrawls", async function () {
          await expect(
            nftMarketplace.withdrawSellerFund()
          ).to.be.revertedWithCustomError(
            nftMarketplaceContract,
            "NftMarketplace__SellerFundNotEnough"
          )
        })
        it("withdraws proceeds", async function () {
          ...
          const deployerProceedsBefore = await nftMarketplace.getSellerFund(
            await deployer.getAddress()
          )
          const deployerBalanceBefore = await deployer.getBalance()
          const txResponse = await nftMarketplace.withdrawSellerFund()
